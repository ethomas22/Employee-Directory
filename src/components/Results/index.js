...
export default function Results(props) {
  ...
  return (
    <div>
        ...
        <div
          ...
          onClick={function () {
            if (employeeResult.sorted) {
              ...
              setemployeeResult({
                results: employeeResult.results
                  .map((result) => ({
                    ...result,
                    toString: function () {
      ...
      {employeeResult.results.filter(result => (result.name.first + " " + result.name.last).toLowerCase().includes(props.search.toLowerCase())).map((result) => (
        <Employee
